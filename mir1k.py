...
class MIR1KDataset(Dataset):
    ...
    def __init__(self,
        ...
        for f in tqdm(sorted(os.listdir(os.path.join(self.data_path, self.pitch_folder)))):
            base_name = f.split('.')[0]
            ...
            file_chunks.append(
                max(0, len(f0) - segment) // self.hop_size + 1)
    ...
    def __getitem__(self, index):
        ...
        x = torchaudio.load(wave_filename, frame_offset=self.pitch_offset + offset * self.pitch_step,
                            num_frames=frames)[0]
        if x.shape[1] < frames:
            x = torch.cat(
                [x, x.new_zeros((x.shape[0], frames - x.shape[1]))], dim=1)
        if f0.numel() < self.segment:
            f0 = torch.cat(
                [f0, f0.new_zeros((self.segment - f0.numel()))], dim=0)
